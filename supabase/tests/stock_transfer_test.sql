...
-- A HANDSTOCK REQUEST NEEDS A REAL NSM APPROVAL (0210). Commercial still
-- auto-clears -- there is no machine behind a HandStock line for Commercial to
-- weigh -- but NSM judges whether the stock is warranted, so the RM may not
-- wave it through. This fixture did, from before 0210 changed the rule, and the
-- refusal then read as an unexpected error in this suite rather than as the
-- guard working.
update public.spare_request_lines set rm_approval='Approved', rm_by='RM Ravi', rm_at=now(),
       commercial_approval='Auto-Approved' where request_uid='H1';
call public.be('nsm@x.com');
update public.spare_request_lines set nsm_approval='Approved', nsm_by='NSM Nita', nsm_at=now()
 where request_uid='H1';
...
select v.u, 'T-VISIT-' || v.u, now(), now() from (values ('U1')) v(u)
