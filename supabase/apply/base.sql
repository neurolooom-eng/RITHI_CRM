...
-- Refuse to run where this bundle would LOSE ground.
do $$
begin
  if to_regclass('public.app_roles') is not null then
    raise exception '%', $refuse$This database is already past the base schema (app_roles is present). Running base.sql here would put the pre-RBAC version of 23 policies and 6 functions back — every user seeing every call among them. Run supabase/apply/all.sql instead; it carries this file and everything after it.$refuse$;
  end if;
end $$;
