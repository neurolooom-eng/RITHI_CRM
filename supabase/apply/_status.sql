...
,
    (169, 'Product Database 2.0', 'product_database_v2 + machine_key() + contract_cover_code() + cover_period_end() (0218). The user, 2026-09-20: "All unique product+serial no should be listed [Warranty Sale Details, Contract Details, Additional Entries], then arrange these sources + Ownership Transfer to come to a conclusion on the Party, Warranty period, contract period, then derive the item status." IT DOES NOT REPLACE public.products OR machine_cover -- the ask was explicit ("Do Not disturb the current product Database") -- so this row checks the NEW view exists and says nothing about the old one. THREE PLACES IT DELIBERATELY DIFFERS FROM machine_cover, each of which is why a second view exists rather than an edit to that one: (1) it keys on PRODUCT + SERIAL, where machine_cover keys on lower(btrim(serial)) alone -- src/lib/machine.ts says why that is wrong, "the install base has eleven machines numbered 219", and a serial-only key merges them into one row wearing one machine''s cover; (2) it reads FIVE registers, where machine_cover reads two -- Additional Entries (0073, the machines recovered by hand precisely because the registers missed them) and Ownership Transfer (0072) are not consulted there at all, so a machine that changed hands still reads as the original buyer''s; (3) WARRANTY DECIDES BEFORE CONTRACT, where machine_cover asks the contract first -- the user''s rule, and it changes the answer for every machine inside both, because a machine in warranty is not being billed under its contract. cover_period_end() reproduces addPeriod()''s JAVASCRIPT month overflow (31-Jan + 1 month is 2-Mar, not 27-Feb): Postgres clamps where JS rolls forward, and 26 of 458 start/period combinations disagree if you use a plain interval -- proved against the app rather than reasoned about. A contract whose type is blank reads CONTRACT (TYPE NOT RECORDED) rather than machine_cover''s guess of CMC, because a guess written into a quality record is worse than a value that reads as odd. NO means the 2.0 view or one of its three functions is missing; the old Product Database is unaffected either way. Restore: product_database_2.sql',
        (to_regclass('public.product_database_v2') is not null
         and to_regprocedure('public.machine_key(text,text)') is not null
         and to_regprocedure('public.contract_cover_code(text)') is not null
         and to_regprocedure('public.cover_period_end(date,numeric)') is not null)),
    (170, 'Product Database 2.0 is MATERIALISED', 'product_database_v2_mv + machine_install_start() + refresh_product_database_2() + product_database_v2_state (0220). REPORTED FROM USE, 2026-09-20: the screen came back empty under "Load failed: canceling statement due to statement timeout". MEASURED on a database built from every migration and loaded to the live project''s order of magnitude, as postgres and again under RLS as authenticated: count(*) 164 ms without RLS and 1,361 ms with it, and ONE PAGE of the screen''s own read (select *, order by machine_key, limit 1000) 2,936 ms. The screen PAGES the view, so all five registers were re-derived for every page -- ten pages is ~30 s, and a single page already exceeds Supabase''s statement timeout on live volumes. WHERE IT WENT: the installation-call lookup alone measured 24 ms without RLS and 1,255 ms with it, FIFTY-TWO TIMES, because calls is the one source whose read policy is per ROW (can_view_all_calls() OR mine OR my team''s, and the team branch walks user_directory recursively) -- the other four have whole-table predicates that cost the same on one row or a million. TWO PARTS, AND THE FIRST IS ABOUT CORRECTNESS: machine_install_start() is SECURITY DEFINER because a machine''s warranty start is a fact about the MACHINE, not about who is looking -- through the per-row policy two people got DIFFERENT warranty dates for the same machine and the one who could not see the installation call silently got the selling register''s date instead. Second, the derivation is materialised: the definer function alone took a page to 1,764 ms, still hopeless, because the cost is re-deriving every machine out of eight tables on every page. With the matview and a unique index on machine_key the same reads measure 5 ms, 3 ms and 6 ms. THE GATE IS KEPT: a matview carries no RLS, so product_database_v2 stays a security_invoker view over it with the SAME whole-table predicate as sale_items_read (masters.view OR cover.edit OR admin), the narrowest of its sources -- proved by measurement, an engineer holding neither reads 0 rows and is refused the rebuild. The matview is granted to NOBODY. THE FIGURES ARE AS OF THE LAST REBUILD and refreshed_at is published as a column so the screen can date them. 0221 THEN REMOVED THE GATE 0220 INVENTED. It was the only screen in its family with a permission predicate of its own, and it broke: a security_invoker view over a REVOKED matview refused everybody, and the user asked the right question -- "how does a Super Admin not have access to something???". They do. Four of the five sources assembled here are already readable by any signed-in user (products -- the OLD Product Database -- product_additional_entries, ownership_transfers and machine_cover, which publishes the same machine and cover facts to everyone), so the screen is gated where every other screen is: by mod:/product-database-2 (0219). NO means either the speed fix or that repair is missing, and on a project that ran 0220 alone the screen REFUSES EVERY READER. Restore: product_database_2.sql',
        (to_regclass('public.product_database_v2_mv') is not null
         and to_regclass('public.product_database_v2_state') is not null
         and to_regprocedure('public.machine_install_start()') is not null
         and to_regprocedure('public.refresh_product_database_2()') is not null
         -- AND THE VIEW CAN READ ITS OWN STORAGE. 0220 revoked this while
         -- leaving the view security_invoker -- which reads AS THE CALLER --
         -- so it refused EVERY reader, administrators included, with
    ...
    (172, 'Solved Without a Report', 'solved_without_report + mod:/missing-visit-reports on the three admin roles (0224). The user, 2026-09-20: "Create a Report - Call is Solved, but Report or Visit Entry is missing - View only for Admins and Super Admins." It is the list that says WHICH visits to re-upload, and it exists because the alternative was loading every report again and hoping. FOUR GAPS, NOT ONE, because each needs a different fix and a report that lumps them together cannot be acted on: no visit row at all; a visit with no visit_at; a visit with no manual_report; and an ENTRY DATE THAT IS THE IMPORT MOMENT. That last one is the interesting case. reports.updated_at is NOT NULL and DEFAULTS TO now(), so a file with no Visit Entry Date does not leave a blank -- it silently takes the moment of the import, and the gap CANNOT be found by looking for a null. A test written to check the null case is what proved it. So the report looks for the signature instead: how many visits share that timestamp TO THE MICROSECOND. Twenty-five visits genuinely entered at the same instant does not happen; a batch load does. The count is published as a column either way, so the reader sees the evidence rather than only the verdict. It matters because updated_at is what decides a call''s status -- 0032 takes the LATEST ENTRY -- so a whole batch sharing one stamp lets an arbitrary row decide every call in it. A call''s row lists EVERY gap it has rather than the first: being told, fixing it, and being told the next is three round trips for one call. "Solved" includes "Solved - Report Pending" and the row says which, because they are different findings -- Report Pending is the system stating a known absence, a plain Solved with no report is the system contradicting itself. THE VIEW INVENTS NO PERMISSION RULE: it is security_invoker, so the ordinary call policies decide the rows, and it is the SCREEN that is restricted, by the module key. FILED IN THE `reports` BUNDLE rather than `daily_review` because a view is resolved AT CREATION and daily_review runs first in ALL_ORDER. NO means the report is missing, or its key reaches nobody -- which on a project in use is the same as missing, since a code default applies only to a role with no stored permissions. Restore: reports.sql',
        (to_regclass('public.solved_without_report') is not null
         and exists (select 1 from public.app_roles
                      where role = 'admin' and permissions ? 'mod:/missing-visit-reports'))),
    (173, 'Data Export', 'exportable_tables() + is_exportable_table() + mod:/data-export on admin AND technical_support (0227, 0228). The user, 2026-09-22: "I need to Export CSV only. Maybe I can select the Tables." The screen lists the public tables with an ESTIMATED row count and downloads the chosen ones as one ZIP of CSV files, READ THROUGH THE ORDINARY API AS THE PERSON SIGNED IN -- so an export holds exactly the rows that person is entitled to see, which is the property that makes it safe to put on a menu. exportable_tables() returns NAMES AND COUNTS ONLY and returns nothing at all to a caller who is not an administrator; the audit trails are withheld from it, because a copy of the record of what everyone did, sitting on a laptop, is a liability rather than a backup. THE COUNTS ARE ESTIMATES FROM THE PLANNER and the screen says so -- a count(*) over sixty tables to paint a picker would scan the database every time it opens, which is 0099 again. IT IS ITS OWN BUNDLE, AND THAT WAS LEARNED THE HARD WAY: filed under call_requests, the screen shipped before its migration was applied and the first person to open it got "Could not find the function public.exportable_tables without parameters in the schema cache", with the only remedy on offer being a replay of a 40-file bundle against a live project -- the thing that has taken locks here before. NO means the screen is live and its database side is not, which is exactly what the user reported. Restore: data_export.sql',
        (to_regprocedure('public.exportable_tables()') is not null
         and to_regprocedure('public.is_exportable_table(text)') is not null
         and exists (select 1 from public.app_roles
                      where role = 'admin' and permissions ? 'mod:/data-export'))),
    (174, 'Scheduled export: what and when, never where', 'export_schedules + export_runs + export_run_due_at() + due_export_schedules() + the zz_export_schedule_guard trigger (0228). The user, 2026-09-22: "Or can we have Scheduled Export option in the UI itself so that i will schedule which ever is necessary." THERE IS NO DESTINATION COLUMN, AND THAT IS THE WHOLE DESIGN. The first attempt held the delivery address in a settings row and was refused as an exfiltration primitive -- rightly: it made the nightly copy of the entire customer base redirectable by any administrator, silently, with nothing on any screen looking different the next morning. So WHICH TABLES and AT WHAT TIME are data an administrator edits, and WHO RECEIVES IT is a secret on the Edge Function (EXPORT_TO), set with the CLI by somebody holding the project keys -- a different person through a different channel. THE SECOND CONTROL IS ON THE NAMES: the job reads with the service role, past RLS, because a schedule has no signed-in person behind it, so an unchecked name here would be a way to mail out record_audit. is_exportable_table() is ONE rule, shared with the picker, applied by a trigger on every insert and update. WHEN A SCHEDULE IS DUE IS SQL RATHER THAN JAVASCRIPT, because the Edge Function is the one part of this system no suite here can run: export_run_due_at() returns the MOST RECENT due instant and the job compares it with last_run_at, so a night the container was restarting is caught up at the next tick instead of lost, and nothing is ever sent twice. Asia/Kolkata throughout -- the job runs in UTC and would otherwise fire five and a half hours early and look correct doing it. export_runs is READ-ONLY through the API even to an administrator: what left the building cannot be tidied from a screen. NO means the schedules cannot be saved, or the poke finds nothing to send. Restore: data_export.sql',
        (to_regclass('public.export_schedules') is not null
         and to_regclass('public.export_runs') is not null
         and to_regprocedure('public.due_export_schedules()') is not null
         and exists (select 1 from pg_trigger
                      where tgname = 'zz_export_schedule_guard' and not tgisinternal)
    ...
    (178, 'An installation call says INSTALLATION CALL, in one spelling', '"INSTALLATION CALL" is on the Standard Complaint master, and no INSTALLATION call is still carrying the old words "Installation Calls" (0233). The user, 2026-09-23: "STANDARD COMPLAINT= INSTALLATION CALL , Reported Complaint= INSTALLATION CALL". THIS ROW EXISTS BECAUSE THE VALUE IS A DIMENSION, not a label: every count, filter and frequent-failure match groups by Standard Complaint, so two spellings do not read as a typo -- they split the total in half and the reader believes both halves. THE MASTER IS THE HALF THAT IS EASY TO FORGET: that field takes NO free text (the user''s standing rule), so a value the master has not got is one nobody can choose and one a call opened in the form cannot show. IT IS SEEDED UNDER WHICHEVER NAME THIS PROJECT USES -- listMaster reads both `complaint` and `standardComplaint` -- because which one a project was seeded with is a fact about the data. THE OLD MASTER ROW IS LEFT WHERE IT IS: it is not a record, but removing it would stop the picker offering a value historical calls still carry, and a filter built on it would silently return nothing. A FIELD CALL SAYING "Installation Calls" IS NOT TOUCHED and this row does not test it -- those are somebody''s own words on their own call, and rewriting a quality record this application did not write is a different act needing a different reason. NO means new calls and old ones disagree, and every installation count is split. Restore: sales_contracts.sql',
        (to_regclass('public.masters') is null
      or (exists (select 1 from public.masters
