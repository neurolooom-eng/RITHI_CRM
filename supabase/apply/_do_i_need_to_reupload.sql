...
-- matched, and would be loaded a second time — row 3 is that count.
-- ===========================================================================
with c as (
  select
    count(*)                                                                  as rows_all,
    ...
    count(*) filter (where coalesce(btrim(source_ref_key), '') = ''
                       and jsonb_typeof(data) = 'object' and data <> '{}'::jsonb) as rows_extra_no_key,
    count(*) filter (where public.imported_ts(data, 'Visit Entry Date') is not null) as rows_with_imported_entry,
    count(*) filter (where exists (select 1 from public.reports r where r.ucn = spare_consumption.ucn)) as rows_with_a_visit
  ...
  lateral (values
    ...
    (4, 'Visit Entry Date — rows where the import already carried it',
        ...
        case when c.rows_with_imported_entry > 0
             then 'Already in. Re-uploading the same file changes nothing — the report reads this value through 0215.'
    ...
    (5, 'Rows with a real visit behind them',
        ...
        'These take their two dates from the visit itself and are unaffected by anything above. The remainder is the closed, shrinking set 0214 stopped growing.'),
