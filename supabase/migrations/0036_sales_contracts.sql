...
declare m record;
begin
  if coalesce(trim(p_serial), '') = '' then return; end if;
  select * into m from public.machine_cover where serial_key = lower(trim(p_serial));
  ...
  where lower(trim(p.serial_number)) = lower(trim(p_serial));
end $$;
...
begin
  for r in select serial_number from public.machine_cover where coalesce(serial_number, '') <> '' loop
    perform public.sync_product_cover(r.serial_number);
    n := n + 1;
  end loop;
  return n;
