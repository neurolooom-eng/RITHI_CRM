...
- **Run at** 2026-09-26T17:50:07.181Z
- **Took** 106s
- **Commit** `2baed3a` on `claude/usage-k7slq0`
- **Version** 0.9.379
...
| | Passed | Total |
| --- | --- | --- |
| Database suites | 105 | 105 |
| Automated checks | 22 | 22 |
| Labelled `expect ERROR` outcomes matched | 197 | 197 |
...
| | Result | |
| --- | --- | --- |
| `271 migrations applied to a fresh database` | ✅ pass |  |

## Automated checks

| | Result | |
| --- | --- | --- |
| `check:bundles` | ✅ pass | no NEW object is split across modules (394 checked, 37 known and listed) |
...
| `check:orders` | ✅ pass |   ✓ 128 order columns across 59 relations |
| `check:paging` | ✅ pass | all passed |
| `check:picklist` | ✅ pass | all passed |
| `check:picklist:open` | ✅ pass | all passed |
| `check:replay` | ✅ pass | every bundle (27) replays with no change to the schema |
| `check:reports` | ✅ pass | all passed |
| `check:safe-updates` | ✅ pass | no WHERE-less update or delete in any of the 227 functions |
| `check:scheduled-export` | ✅ pass | all passed |
| `check:status` | ✅ pass | every one of the 200 _status.sql rows reads yes on a fully-applied database (1 skipped) |
