...
- **Run at** 2026-09-26T12:11:42.388Z
- **Took** 128s
- **Commit** `68f6ed5` on `claude/usage-k7slq0`
- **Version** 0.9.375
...
| | Passed | Total |
| --- | --- | --- |
| Database suites | 103 | 103 |
| Automated checks | 22 | 22 |
| Labelled `expect ERROR` outcomes matched | 186 | 186 |
...
| | Result | |
| --- | --- | --- |
| `266 migrations applied to a fresh database` | ✅ pass |  |

## Automated checks

| | Result | |
| --- | --- | --- |
| `check:bundles` | ✅ pass | no NEW object is split across modules (392 checked, 37 known and listed) |
| `check:columns` | ✅ pass | every column of all 33 registers exists |
| `check:cover-party` | ✅ pass | all passed |
| `check:dberror` | ✅ pass | all passed |
| `check:generated` | ✅ pass | every generated bundle matches its migrations (93 checked) |
...
| `check:safe-updates` | ✅ pass | no WHERE-less update or delete in any of the 225 functions |
| `check:scheduled-export` | ✅ pass | all passed |
| `check:status` | ✅ pass | every one of the 196 _status.sql rows reads yes on a fully-applied database (1 skipped) |
