...
```
Stock Level = Stock Out (from Stores) − Consumption − Stock Transfer From + Stock Transfer To
```
...
`transferred_out` and `on_hand` on the same row.
*Verified by:* `handstock_balance` exposes all five columns;
`handstock_test.sql` step 6.

**HS-3.** Nothing is entered to maintain a stock level. All four movements
...
`scripts/build-apply-bundles.mjs`; carries `0023_handstock.sql`), ending with
