...
function judge(name, out) {
  ...
  for (const raw of lines) {
    const line = raw.trim();
    if (/expect\s+ERROR/i.test(line)) { pending.push(line); expectations += 1; continue; }
...
const NEEDS_DB = { 'check:views': 'db', 'check:status': 'db', 'check:upserts': 'db', 'check:replay': 'nodb' };
const checkDb = `val_checks`;
if (results.setup[0]?.ok) { try { admin(`drop database if exists ${checkDb};`); admin(`create database ${checkDb} template ${TPL};`); } catch { /* reported below */ } }
