...
// Stores dispatch has no patch builder on purpose. A stock out is a BATCH —
// many spares, one engineer, one DC — and its numbers are generated by
// Postgres (dispatch_spare_lines(), 0027_spare_dispatch.sql), so there is one
// way to book stock out and no way to type a DC number by hand. See
// src/lib/sparedispatch.ts and the Pending Dispatch screen.
